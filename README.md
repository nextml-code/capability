...
### Discrete relaxation
Much like the Gumbel-softmax trick, that tries to let us get gradients through a discrete transformation, there is the idea that we can replace the derivative of a discrete function during training and analyze what happens mathematically. This is useful for [sequential modeling](https://arxiv.org/pdf/1801.09797.pdf) and for creating better [variational autoencoders](https://arxiv.org/pdf/1906.00446.pdf). We implemented a couple of versions of our own and [the original](https://github.com/Aiwizo/capability/blob/master/discrete_relaxation/kaiser_step.py) to try on some simple problems.
